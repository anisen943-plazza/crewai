...
def create_knowledge_source():
    """Create a knowledge source from the analysis results file with enhanced metadata."""
    knowledge_file = 'sales_analysis_results.md'  # Relative path - will be prefixed with 'knowledge/'
    parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    knowledge_path = os.path.join(parent_dir, 'knowledge', knowledge_file)
    
    # Ensure knowledge directory exists
    os.makedirs(os.path.dirname(knowledge_path), exist_ok=True)
    
    # Check if the file exists in either location
    file_exists = os.path.exists(knowledge_path) or os.path.exists(ANALYSIS_RESULTS_PATH)
    ...
    if file_exists:
        ...
        metadata = {
            ...
            "data_domains": ["sales", "inventory", "customer", "product"],
            "version": "1.0",
            "last_modified": os.path.getmtime(knowledge_path) if os.path.exists(knowledge_path) else "new"
        }
        
        # Use relative path that will work with CrewAI's knowledge directory structure
        # Improved chunking parameters based on document structure
        markdown_source = TextFileKnowledgeSource(
            file_paths=[knowledge_file],  # Just the filename, CrewAI will prepend "knowledge/"
            chunk_size=800,               # Optimal size for maintaining section coherence
            chunk_overlap=200,            # Sufficient overlap to maintain context across chunks
            metadata=metadata
        )
        ...
        print(f"Warning: No knowledge file found. Creating empty knowledge source.")
        # Return empty Knowledge object instead of None for better consistency
        return Knowledge(
            collection_name="sales_analysis",
            sources=[]
        )
...
def save_analysis_results(content, filepath=ANALYSIS_RESULTS_PATH):
    ...
    Implements improved content structuring and metadata for better retrieval.
    ...
    try:
        ...
        if hasattr(content, 'raw'):  # CrewOutput object
            content = content.raw
        elif not isinstance(content, str):  # Other non-string type
            content = str(content)
        
        # Format the content with clear section headers and metadata for better chunking
        if isinstance(content, str):
            # Add main heading if not present
            if not content.startswith("# "):
                content = f"# Sales Analysis Results - {timestamp}\n\n{content}"
            
            # Add metadata section for better retrieval if not present
...
"""
                # Insert metadata after title but before content
                parts = content.split("\n\n", 1)
                if len(parts) > 1:
                    content = f"{parts[0]}\n{metadata_section}\n{parts[1]}"
                else:
        ...
        # Check if we need to preserve previous analysis history with improved logic
        prev_content_to_preserve = ""
        if os.path.exists(filepath):
            # Read existing content
            with open(filepath, 'r') as file:
                existing_content = file.read()
                
            # Only process if existing content is different
            if len(existing_content) > 0 and existing_content != content:
                # Extract date from previous analysis with more robust pattern
                import re
                ...
                if match:
                    ...
                    try:
                        ...
                        now = datetime.now()
                        days_diff = (now - prev_datetime).days
                        
                        if days_diff > 30:
                            # Keep only the new content
                            print(f"Previous analysis is {days_diff} days old. Replacing with new analysis.")
                        else:
                            # Extract the old analysis, skipping metadata section for cleaner history
                            sections = re.split(r"##\s+\w+", existing_content)
                            if len(sections) > 1:
                                # Skip metadata section if present
                                if "Metadata" in existing_content:
                                    # Find the content after metadata
                                    content_sections = existing_content.split("## Metadata")[1].split("##")
                                    if len(content_sections) > 1:
                                        prev_content_to_preserve = "##".join(content_sections[1:])
                                    else:
                                        prev_content_to_preserve = existing_content
                                else:
                                    prev_content_to_preserve = existing_content.split("# ")[1] if "# " in existing_content else existing_content
                            
                            print(f"Preserving previous analysis ({days_diff} days old) in the file.")
                    except Exception as e:
                        print(f"Could not parse previous date: {e}")
        
        # Add historical content if available
        if prev_content_to_preserve:
            content = f"{content}\n\n## Previous Analysis\n\n{prev_content_to_preserve}"
        ...
        # Also save to knowledge directory with better directory handling
        parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        ...
        with open(knowledge_file, 'w') as file:
            file.write(content)
        print(f"Analysis results saved to knowledge directory for RAG integration")
        
    except Exception as e:
        print(f"Error saving analysis results: {str(e)}")
        # Try to save with minimal formatting as fallback
        try:
            with open(filepath, 'w') as file:
                file.write(f"# Sales Analysis Results - {timestamp}\n\n{str(content)}")
            print(f"Saved minimal version due to error in full processing")
        except:
            print(f"Critical error: Could not save results at all")
...
try:
    ...
    parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    visuals_dir = os.path.join(parent_dir, "visuals")
    viz_paths = visualize_analysis_results(ANALYSIS_RESULTS_PATH, output_dir=visuals_dir)
    print(f"Visualizations generated and saved to {viz_paths.get('enhanced_markdown', visuals_dir)}")
except Exception as e:
